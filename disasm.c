...
// 反汇编1,2或4字节通用整数寄存器的名称，如果可用，则转储其内容
...
// 服务函数，将MASM或Ideal格式的有效内存地址添加到反汇编的字符串中。 
//参数
//defseg   - 给定寄存器组合的默认段
//descr    - 地址的完全解码寄存器部分
//offset   - 地址的常量部分
//dsize    - 以字节为单位的数据大小
//如果设置了全局标志'symbolic'，则函数还会尝试将偏移解码为某个标签的名称。
static void Memadr(int defseg,const char *descr,long offset,int dsize) 
{
	...
	//在某些情况下，反汇编程序可能会忽略内存操作数的大小
	//即，标志showmemsize必须为0
	//类型位C_EXPL必须为0（该位即表示需要显式操作数大小）
	//并且命令类型不能是C_MMX或C_NOW（因为位C_EXPL在这些情况下具有不同的含义）
	//否则，必须提供确切的尺寸
...
//从ModRM/SIB字节中反汇编内存/寄存器，如果可用，则转储内存的地址和内容
...
// 解码大小constsize的立即操作数
// 如果sxt非零，则应将字节操作数符号扩展为sxt字节
// 如果立即常数的类型假定为此，则小的负操作数可以显示为带符号的负数
// 请注意，在大多数情况下，注释窗口中不会显示立即操作数
static void DecodeIM(int constsize,int sxt,int type) 
{
	int i;
	signed long data;
	ulong l;
	char name[TEXTLEN] = { 0 }, comment[TEXTLEN] = { 0 };//修复问题
	...
	// 作为Intel规则的一个例外，Command ENTER有两个立即常量
	// 由于第二个常量很少使用，如果第一个常量非零（通常是这种情况），我会将其从搜索中排除。
	if (da->immconst == 0)
		da->immconst = data;
  if (mode>=DISASM_FILE && da->error==DAE_NOERR) 
  {
...
// 解码立即绝对数据地址
// 该操作数用于8080兼容命令，允许将数据从内存移动到累加器和后端
// 请注意，字节ModRM和SIB永远不会出现在带有IA操作数的命令中
...
static void DecodeRJ(ulong offsize,ulong nextip) 
{
	...
	if (mode >= DISASM_FILE)
	{
		if (offsize == 1) nresult += sprintf(da->result + nresult, "%s ", (lowercase == 0 ? "SHORT" : "short"));
		if (mode >= DISASM_CODE)
...
// 解码立即绝对远跳地址
// 在平面模型中，不使用这样的地址（主要是因为选择器直接在命令中指定），所以我既不解码为符号也不对其进行注释
// 为了允许按值搜索选择器，我将其解释为立即数。
...
// 解码ModRM字节的R部分中寻址的控制寄存器。 这种类型的操作数极为罕见。 
// 控制寄存器的内容只能从权限级别0访问，所以我不能在这里转储它们。
...
// 解码在ModRM字节的R部分寻址的调试寄存器。 
// 这种类型的操作数极为罕见。 我只能转储CONTEXT结构中可用的调试寄存器
...
// 0F, 0F).
// 跳过3DNow！ 操作数和提取命令后缀
// 如果后缀位于内存块之外，则返回后缀或-1 
// 此子例程假定cmd仍然指向3DNow的开头！ 
// 命令（即到两个字节0F，0F的序列）。
static int Get3dnowsuffix(void)
{
	...
	if ((c & 0xC0) == 0xC0)
		;
	// 16-bit addressing mode, SIB byte is never used here.16位寻址模式，此处从不使用SIB字节
	else if (addrsize == 2)
	{
...
// not possible).
// 函数检查80x86标志是否满足命令中设置的条件
// 如果满足条件则返回1，如果不满足则返回0，如果出现错误则返回-1 这是不可能的
